...
router.post('/settings', (request, response) => {
	...
	if(request.body.newUserName) (filter.userName = request.body.newUserName) && (attributes.push('userName'))
	if(request.body.newEmail) (filter.email = request.body.newEmail) && (attributes.push('email'))
	if(request.body.newFullName) (filter.fullName = request.body.newFullName) && (attributes.push('fullName'))
	...
	db.user.findOne({
		...
		updateFilter.update(filter
			...
			db.user.findOne({
				...
				response.render('settings', {message: "Succesfully updated your settings.", user: updateUser})
			})
		})
	})
})
